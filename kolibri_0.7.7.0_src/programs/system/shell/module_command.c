...
trim(args);
	...
	for (i = 0; i < sizeof(COMMANDS)/sizeof(COMMANDS[0]); i++)
	{
		...
		{
			if (COMMANDS[i].numargs == 0)
				((handler0_t)COMMANDS[i].handler)();
			else if (COMMANDS[i].numargs < 0)
				((handler1_t)COMMANDS[i].handler)(args);
			else
				((handler1_t)COMMANDS[i].handler)(-1 == arg ? NULL : args);
			return;
		}
	}
