...
void* __cdecl memset(void* s, int c, unsigned long n)
{
	unsigned long i;
	for (i = 0; i < n; i++)
		((char*)s)[i] = (char)c;
...
{
	unsigned lenMain,lenEnd;
	COptimal* opt,*prevOpt;
	unsigned reps[kNumRepDistances];
	unsigned repLens[kNumRepDistances];
	unsigned repIndex,repMaxIndex;
	unsigned i,len,repLen,lenTest,newLen,lenTestTemp,lenTest2;
	unsigned posState,posStateNext;
	...
	unsigned cur,posPrev,pos;
	CState state,state2;
	const byte* data;
	bool nextIsChar;
	unsigned numAvailableBytesFull,numAvailableBytes;
	unsigned backOffset,offset;
	unsigned limit;
	...
	while (1)
	{
		...
		if (numAvailableBytes >= 3 && !nextIsChar)
		{
			...
			if (lenTest>=2)
			{
				unsigned posStateNext,nextRepMatchPrice;
				state2 = state;
